...
- setShader (Shader shader)  着色器：用于对图形的填充。Shader有3种模式（TileMode.CLAMP|REPEAT|MIRROR）填充剩余空间，其中填充y轴方向，再填充x方向；Shader与Canvas绘制多大的图形，在哪里绘制都无关，因为**Shader永远从控件原点开始绘制**。
...
> 类似于Socket，一个操作Bitmap的接口。Canvas通过API对Bitmap进行绘制。
...
- drwPath()：绘制路径
  ...
  **Path：一个线、曲线的集合。可以使用Stroke或Fill填充模式。文字可以绘制到路径上**
  ...
  **PathMeasure：专门对Path辅助计算的类，例如可以计算出Path上任意一点的坐标**

  API

  - ```java
    PathMeasure(Path path, boolean forceClosed)//构造函数
    ```

    forceClosed：是否计算最后的闭合路径。若为true，即使Path未闭合也会多计算上这段闭合路径

  - ```java
    public boolean getSegment(float startD, float stopD, Path dst, boolean startWithMoveTo)
    ```

    startD/stopD：沿Path Direction截取距起始点startD距离，距终点stopD距离的片段

    dst：将截取片段**添加**到dst Path中

    startWithMoveTo：false——截取的片段们首尾相连（一般设为true）。
...
> Drawable和View的区别在于不能接收事件，无法与用户交互。Drawable中的Canvas是由View传递过来的，通过提前在Drawable中绘制图像，使得在View在drawBackground时绘制Drawable。

自定义Drawable步骤：

1. 实现抽象方法——

   【一】getIntrinsicHeight/Width() 代表默认大小，若Drawable绘制Bitmap，在应该是bitmap的宽高；
...
1. 视图动画包括4个基本的补间动画（android.view.animation）和1个帧动画，只能所用于View上，且动画的过程无法真实修改View的属性值

##### 属性动画

1. 属性动画（android.animation）脱离于View之外，作用于特定的属性值。

> 开发注意点：
>
> 1. ofXXX构造函数——【一】若只写了一个参数，则ObjectAnimator 会通过查找对应属性的 getProperty 函数来获得初始值，若没有则会取基本数据类型的缺省值；【二】指定的ofFloat参数在动画过程中只会大概执行到，在INFINITE模式下，初始值和结束值也不会执行到；【三】ofInt、ofFloat是有默认的evaluator，而ofObject需要开发者提供对应的TypeEvaluator
> 3. xml 写法需要写明valueType
> 4. propertyName是如 setTranslationX 除去set之后的单词，无论首字母大小写都可以
...
- **attrs文件 declare-styleable的标签和属性 与代码获取对应属性值**
  ...
  3. fraction：百分数——50%
  4. enum：枚举——如orientation
  5. flag：位运算——如 gravity（enum只有一种类型，flag也是类型但是类型可以叠加）
...
> 大致流程：`ViewGroup A` 和`View B`。
>
> 1. 事件由A传给B，这之间A有权对事件拦截。
> 2. A若开始拦截事件（无论任何事件类型），则B永远不会再收到事件（除了cancel）；A若不拦截，则事件交给B处理。
> 3. B若处理了事件（`B.dispatchTouchEvent return true`），则事件传递：A -> B；若B没有处理，则事件传递：A -> B ->A

**结合开发场景分析事件传递机制**

1. **A不拦截的情况**

- B消费`down`事件，及后续`move、up`事件传递过程
  ...
  B消费`Down`事件后，`mFirstTouchTarget`初始化并保存B，轮不到A.`onTouchEvent()执行`，之后的`Move`和`Up`事件直接通过`mFirstTouchTarget`传递给B。（即使B未消费`Move`、`Up`事件，事件也会分发给B）
...
> 1. 首先`ScrollBy`和`ScollTo`平移的是`View`的`Canvas`。
> 2. `ScrollTo(x,y)`是平移到指定的`(x,y)`坐标。`View.mScrollX = x`，`View.mScrollY = y`
> 3. `ScrollBy(x,y)`是相较于当前`(mScrollX,mScrollY)`做平移。`View.mScrollX += x`，`View.mScrollY += y`

**computeScroll()**：

> 一般用于实现滑动递进式回退效果。如ViewPager拖拽后松手。
...
- 实现
  ...
  利用`View.invalidate()`触发`computeScroll()`回调，修改`mScrollX、mScrollY`
